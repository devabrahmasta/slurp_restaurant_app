...
class LocalNotificationService {
  ...
  Future<void> scheduleDailyElevenAMNotification({
    ...
    final response = await api.getRestaurantList();
    if (response.error || response.restaurants.isEmpty) return;

    final random = Random();
    final randomIndex = random.nextInt(response.restaurants.length);
    final randomResto = response.restaurants[randomIndex];
    ...
    await flutterLocalNotificationsPlugin.zonedSchedule(
      ...
      body: 'Yuk mampir ke ${randomResto.name} di kota ${randomResto.city}.',
      scheduledDate: datetimeSchedule,
      notificationDetails: notificationDetails,
      androidScheduleMode: AndroidScheduleMode.exactAllowWhileIdle,
      matchDateTimeComponents: DateTimeComponents.time,
      payload: randomResto.id,
    );
  }
